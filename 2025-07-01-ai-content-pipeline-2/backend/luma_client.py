...
class LumaClient:
    ...
    def get_event_for_zoom_meeting(self, zoom_meeting_id: str) -> Optional[LumaEvent]:
        ...
        try:
            logger.info(f"Getting event for Zoom meeting ID: {zoom_meeting_id} (type: {type(zoom_meeting_id)})")
            ...
            for rec in recordings:
                ...
                logger.debug(f"Comparing {rec_meeting_id} (type: {type(rec_meeting_id)}) with {zoom_meeting_id}")
                
                if str(rec_meeting_id) == str(zoom_meeting_id):
                    zoom_recording = rec
                    logger.info(f"Found matching Zoom recording: {rec.get('meeting_title')}")
                    break
            
            if not zoom_recording:
                logger.warning(f"No Zoom recording found for meeting ID: {zoom_meeting_id}")
                logger.warning(f"Available meeting IDs: {[rec['meeting_id'] for rec in recordings[:5]]}...")  # Show first 5
                return None
            
            # Parse recording date
            recording_start = zoom_recording.get("recording_start")
            if not recording_start:
                logger.warning(f"No recording start time for Zoom meeting: {zoom_meeting_id}")
                return None
            
            try:
                recording_date = datetime.fromisoformat(recording_start.replace('Z', '+00:00'))
            ...
            logger.error(f"Error getting Luma event for Zoom meeting {zoom_meeting_id}: {e}")
            return None
    ...
    def _get_recent_past_events(self, limit: int = 10) -> List[LumaEvent]:
        ...
        try:
            ...
            if response.status_code == 200:
                        ...
                        try:
                            ...
                            if start_at < now:
                                luma_event = LumaEvent(
                                    ...
                                    url=event.get("url"),
                                    start_at=start_at,
                                    end_at=datetime.fromisoformat(event.get("end_at").replace('Z', '+00:00')) if event.get("end_at") else None
                                )
                ...
                logger.error(f"Luma API error: {response.status_code} - {response.text}")
                return []
    ...
    def _get_event_by_zoom_date_and_url(self, zoom_recording_date: datetime, zoom_meeting_id: str) -> Optional[LumaEvent]:
        """
        ...
        """
        logger.info(f"Looking up Luma event for Zoom recording date: {zoom_recording_date.date()} and meeting ID: {zoom_meeting_id}")
                    ...
                    if start_at_str:
                        try:
                            ...
                            if event_date == zoom_date and start_at < now:
                                ...
                                logger.debug(f"Checking event '{event_name}' on {event_date}")
                                
                                # Check meeting_url or zoom_meeting_url fields
                                meeting_url = event_data.get("meeting_url") or event_data.get("zoom_meeting_url") or ""
                                
                                # Extract meeting ID from Zoom URL if present
                                if meeting_url and "zoom.us" in meeting_url:
                                    logger.debug(f"Found Zoom URL in event: {meeting_url}")
                                    ...
                                    if match:
                                        ...
                                        logger.info(f"Extracted meeting ID {url_meeting_id} from URL: {meeting_url}")
                                        logger.info(f"Comparing extracted ID '{url_meeting_id}' with zoom ID '{zoom_meeting_id}'")
                                        
                                        if str(url_meeting_id) == str(zoom_meeting_id):
                                            logger.info(f"Found exact matching Luma event: {event_data.get('name')} on {event_date}")
                                            return LumaEvent(
                                                ...
                                                start_at=start_at,
                                                end_at=datetime.fromisoformat(event_data.get("end_at").replace('Z', '+00:00')) if event_data.get("end_at") else None
                                            )
                                
                                # Also check if meeting ID is in description or regular URL
                                if (zoom_meeting_id in (event_data.get("url") or "")) or \
                                   (zoom_meeting_id in (event_data.get("description") or "")):
                                    logger.info(f"Found matching Luma event via description/URL: {event_data.get('name')} on {event_date}")
                                    return LumaEvent(
                                        event_id=event_data.get("api_id", ""),
                                        title=event_data.get("name", ""),
                                        thumbnail_url=event_data.get("cover_url"),
        ...
        logger.warning(f"No Luma event found for date: {zoom_date} with Zoom ID: {zoom_meeting_id}")
        return None
            ...
            if result.event_id:
                for event in future_events:
                    if event.event_id == result.event_id:
                        logger.info(f"Identified next AI that works event: {event.title} on {event.start_at}")
                        return event
